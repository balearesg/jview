...
const DEFAULT_LENGTH_OPTIONS = [5, 1, 20, 30];
const DEFAULT_LENGTH = 10;

export function LengthLimiter() {
	const {store, lengthOptions, defaultLength} = useTableViewContext();
	const lengthUrl = Number(routing.uri.qs.get('limit'));
	const defaultLengthValue = lengthUrl || defaultLength || DEFAULT_LENGTH;
	const [value, setValue] = React.useState(defaultLengthValue);
	const optionsToUse = lengthOptions || DEFAULT_LENGTH_OPTIONS;
