...
export function View(): JSX.Element {
	const { container, setShow, setStates, setValues, show, values, tables, setInit, save, states, entity } =
		usePanelContext();
	const handleClick = (): void => setShow(!show);
	const handleSubmit = async (event: SyntheticEvent<HTMLFormElement, Event>): Promise<void> => {
		event.preventDefault();
		const validCheck: boolean = tables.some((table): boolean => values[table.id] === true);

		if (!validCheck) return;
		const newConfig: Array<table> = tables.filter((table): boolean => values[table.id] === true);
